...
## Ещё две тихие поломки 2.0 (найдены 05.09 вечером)

**Личные сообщения всех собеседников падают в одну сессию `agent:main:main`.** В 2.0 по
умолчанию `session.dmScope = "main"`: DM владельца и любого другого допущенного человека
живут в общем контексте (у нас в гостевом боте смешались переписки двух людей), а
`sessions_spawn`/субагенты из чата падают с `unknown parent session:
agent:main:telegram:default:direct:<id>` — инструмент ищет личный ключ, которого в
хранилище нет. Лечение: `"session": {"dmScope": "per-account-channel-peer"}` + рестарт гейтвея — именно этот
формат (`agent:main:telegram:default:direct:<id>`) ищет инструмент и именно такие ключи были до 2.0;
`per-peer` даёт `agent:main:direct:<id>` и spawn не чинит. Побочный эффект: чат возвращается в
старую личную сессию, накопленный после апгрейда контекст общей сессии остаётся только в `recall`.

**Встроенный инструмент `tts` без провайдера.** Агент видит `tts`, вызывает, получает
«no provider registered», потом начинает «предлагать изменения конфига», которые требуют
одобрения в панели. Если голос делает свой скилл (`addons/skills/say`: piper локально,
русский + итальянский, `--lang`), добавьте `tts` в `tools.deny` обоих агентов.

**Голосовое «не дошло» при верном файле.** Модель переписывает путь `/tmp/tmpXXXX.ogg` как
`/tmpXXXX.ogg` (схлопывает `tmp/tmp`) и `MEDIA:` уходит в никуда. В `say.py` файлы теперь
`/tmp/voice-<время>.ogg`, а правило скилла — отправлять самим скриптом, без `--keep`.

**excash за BunnyCDN режет прямые тела запросов больше ~10 КБ** (400 с HTML-страницей, не зависит от User-Agent), а через
страж-прокси `addons/excash-guard` (aiohttp) проходят и 110 КБ. Все скрипты, которые шлют большие промпты (стенограммы, письма,
картинки base64), ходят через `http://127.0.0.1:8788` с прямым адресом как запасным. Долгие генерации (конспект на 20k+ токенов)
без потока ловят 504 у провайдера — `stream: true` и сборка дельт. Контейнер GigaAM с лимитом 900 МБ падает на 10-минутных
кусках — режем по 4 минуты.

**Скорость: у excash есть `gpt-5.3-codex-spark` (Codex Spark на железе Cerebras) — замер через страж 2400 ток/с против 290 у
gemini-3.8-flash и 30–36 у «-fast» Astra/Sol (у тех «fast» — про очередь). Контекст 128k, картинки. На него переведены конвейеры:
чистка стенограмм, письма научного брифа, классификаторы обещаний/долгов и дайджест (качество там как у flash/Luna, скорость в 30 раз).
Субагенты (многошаговый поиск, рассуждения) — Luna 1M (58 ток/с, но рассуждает и держит 1M); чат — Astra, длинные конспекты — Astra 1M.
Сам Cerebras с датацентрового/RU IP закрыт Cloudflare (403 напрямую и через WARP), на публичном API только gpt-oss-120b и
qwen-3.8-27b, бесплатный тариф 30k TPM — для агентов бесполезен.
