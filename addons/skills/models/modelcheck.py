...
Различаем четыре состояния, и это разные действия:
  ...
  не подключена       — есть в каталоге, но не роутится («Unable to select a model»)
  нет у провайдера    — «не разрешена для SK-ключа». ВНИМАНИЕ: тот же ответ
                        приходит на любое выдуманное имя, так что существование
                        модели он НЕ доказывает
...
def probe(key, model):
    ...
    try:
        ...
        try:
            ...
            body_txt = str(e)[:200]
        low = body_txt.lower()
        if "unable to select a model" in low or "model_routing" in low:
            return "не подключена", "объявлена в каталоге, но не роутится", dt
        if "model_not_available" in low or "не разрешена" in low:
            # ОСТОРОЖНО: этот же ответ приходит на ЛЮБОЕ незнакомое имя — проверено
            # 03.09 на «ыфваопрж-9000». Отличить «есть, но вне тарифа» от «нет вовсе»
            # по ответу НЕЛЬЗЯ. Существование доказывает только каталог.
            return "нет у провайдера", "неизвестна или вне тарифа — по ответу не различить", dt
...
def cmd_check(a):
    ...
    # Имена вне каталога держим под наблюдением, но помним: «нет у провайдера»
    # тут ничего не доказывает. Сигнал — только переход в «работает».
    EXTRA = ["gemini-3.8-flash-tiered", "gemini-3.8-pro",
             "gpt-6", "gpt-6-astra", "gpt-6-astra-fast", "gpt-6-mini"]
    for e in EXTRA:
        if e not in models:
            models.append(e)
...
def cmd_list(a):
    ...
    print("Проверено: %s\n" % st.get("checked"))
    order = {"работает": 0, "не подключена": 1, "лежит": 2, "нет у провайдера": 3, "ошибка": 4}
